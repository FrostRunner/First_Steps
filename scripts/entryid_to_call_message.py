...
def id_to_guid(data):
    data_group = []
    data_group.append("%s%s%s%s" % (data[6:8], data[4:6], data[2:4], data[0:2]))
    data_group.append("%s%s" % (data[10:12], data[8:10]))
    data_group.append("%s%s" % (data[14:16], data[12:14]))
    data_group.append(data[16:20])
    data_group.append(data[20:])
...
def search_replid(mailbox_number, mail_guid):
    ...
    for line in p.stdout:
        ...
        search_items = re.search(reg, line)
        if search_items:
            repl = int(search_items.group(1))
            repr = str.upper(format(repl, '02x'))
            return str(repr)
